...
SERVER_DESCRIPTION = (
    ...
    "Before writing SQL. Read get_table_context with purpose: query for the "
    "tables the query touches - the DDL, the Joins list, the value lists with "
    "their counts, the data dictionary. The profile purpose carries statistics "
    "that describe the data rather than what a predicate needs. "
    "Join through the Joins list: it carries the DDL's foreign keys and the "
    "edges the catalog never declared, each marked declared, inferred or "
    "measured - an inferred edge is a guess from a column name and a measured "
    "one a value containment seen at the read; neither is a constraint, and a "
    "declared edge wins where one exists.\n\n"
    "Before writing a literal. Where a filter needs a value on a text column "
    "and the phrase in the question is not one of the values listed in full, "
    "call resolve_value and use the spellings it returns - all of them when "
    "it returns several, because they are one category stored several ways. "
    "Do not call it for a value the context already lists in full: the lookup "
    "costs a turn and adds nothing there.\n\n"
